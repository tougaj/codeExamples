...
- **CERTIFICATES_ROOT_PATH** — кореневий каталог сертифікатів (по замовчанню `./`). Даний параметр є корисним, коли в наявності кілька серверів і сертифікати для кожного з них розташовано в окремих каталогах.
- **USER_LOGIN** — логін користувача для доступу до серверу (по замовчанню `test`).
- **USER_PASSWORD** — пароль користувача для доступу до серверу (по замовчанню `test`).
