...
llm = LLM(
    ...
    # max_model_len=3000,  # максимальна довжина контексту for 4b
...
def prepare_chat_prompt(text: str) -> str:
    ...
    messages = [
        {
            "role": "user",
            "content": f"""Ти — асистент для обробки текстів. Твоє завдання: прийняти будь-який текст і створити результат у форматі JSON.

Відповідай **тільки у форматі JSON**, без жодних пояснень чи додаткового тексту.

Структура відповіді:

{{
  "summary": "<стисле резюме українською мовою, 2–3 речення>",
  "translation": "<повний переклад вхідного тексту українською мовою>"
}}

Інструкції:
- "summary" — це короткий виклад головної ідеї тексту українською.
- "translation" — повний переклад тексту українською.
- Якщо вхідний текст уже українською — залиш його як переклад, але все одно створи коротке резюме.
- Не додавай нічого, крім правильного JSON (без коментарів, пояснень чи підписів).
...
#         {
#             "role": "user",
#             "content": f"""Проаналізуй наведений нижче текст новинної статті та визнач до п’яти ключових тем, які в ній розглядаються. Подай відповідь **українською мовою**, **без пояснень, нумерації, вступних слів чи додаткового тексту** — лише перелік тем, кожна з нового рядка.
# Текст статті:\n\n{text}""",
#         }
#     ]
#     messages = [
