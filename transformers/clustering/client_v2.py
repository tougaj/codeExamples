...
class ClusteringRequest(BaseModel):
    ids: list[MessageId] = Field(..., min_length=1)
...
class ClusteringTextRequest(BaseModel):
    messages: list[MessageForClustering] = Field(..., min_length=1)
    min_cluster_size: int = Field(default=5, ge=2)
    min_samples: int | None = Field(default=None, ge=1)
    include_noise: bool = False
...
def request_embeddings(texts: list[str]) -> np.ndarray:
    ...
    response = requests.post(
        f"{EMBEDDING_SERVER_ADDRESS}/embed",
    ...
    if not data.normalized:
        raise ValueError("Embedding server returned non-normalized vectors")
    print(
        "ℹ️ Embedding metadata: "
        f"model={data.model}, dimension={data.dimension}, "
        f"prompt={data.prompt!r}, version={data.version}"
    )
    return np.asarray(data.embeddings, dtype=np.float32)


def request_clusters(
    ids: list[str],
    ...
    response = requests.post(
        f"{EMBEDDING_SERVER_ADDRESS}/clusters",
        json=request_data.model_dump(),
        timeout=REQUEST_TIMEOUT,
    )
    if response.status_code != 200:
        raise ValueError(response.text)
    return ClusteringResponse.model_validate(response.json()).clusters


def request_text_clusters(
    ...
    request_data = ClusteringTextRequest(
        messages=[
            MessageForClustering(id=msg.id, text=msg.get_text(MAX_TEXT_LEN))
            for msg in messages
        ],
        min_cluster_size=min_cluster_size,
        min_samples=min_samples,
    ...
    response = requests.post(
        f"{EMBEDDING_SERVER_ADDRESS}/text_clusters",
        json=request_data.model_dump(),
        timeout=REQUEST_TIMEOUT,
    )
    if response.status_code != 200:
...
def estimate_min_cluster_size(messages_count: int) -> int:
    if messages_count <= 1024:
        min_cluster_size = math.log2(messages_count)
    elif messages_count <= 2048:
        min_cluster_size = messages_count/100
    else:
        min_cluster_size = messages_count/133
    return max(2, math.floor(min_cluster_size))
...
def main():
    ...
    if min_cluster_size == 0:
        min_cluster_size = estimate_min_cluster_size(messages_count)
